...
import com.ain.trading.api.BrandsData
...
class BrandTLAdapter(var mFeedsList: MutableList<BrandsData>, val mListener: OnchildCallBack) : RecyclerView.Adapter<BrandTLAdapter.FeedsVH>() {
    var mChildrenList: MutableList<Children> = ArrayList<Children>()
    override fun onCreateViewHolder(parent: ViewGroup, p1: Int): FeedsVH {
        val view = LayoutInflater.from(parent?.context).inflate(R.layout.frag_brand_tl_items, parent, false)
        return FeedsVH(view)
    }
    ...
    override fun onBindViewHolder(holder: FeedsVH, position: Int) {
        ...
        if(!mFeedsList[position].image_path.isNullOrEmpty()) {
            Picasso.with(MyApplication.instance).load(mFeedsList[position].image_path).fit()
                ...
                .into(holder?.brandTLIV)
        }
        holder?.itemView.setOnClickListener {
            mListener.onChildClick(position,mFeedsList[position].brand_id.toString(),mFeedsList[position].brand_title)
        }
...
/*        holder?.itemView.setOnClickListener {
    ...
    inner class FeedsVH(itemView: View?) : RecyclerView.ViewHolder(itemView!!) {
        ...
        var brandTLIV = itemView?.findViewById<ImageView>(R.id.brandTLIV)
