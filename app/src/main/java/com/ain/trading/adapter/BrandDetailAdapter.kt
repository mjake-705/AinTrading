...
import com.ain.trading.api.BrandDetailData
...
class BrandDetailAdapter(var mFeedsList: MutableList<BrandDetailData>, val mListener: OnchildCallBack) : RecyclerView.Adapter<BrandDetailAdapter.FeedsVH>(),
    ...
    override fun onBindViewHolder(holder: FeedsVH, position: Int) {
        ...
        holder?.itemView.setOnClickListener {
            mListener.onChildClick(position,mFeedsList[position].product_id.toString(),mFeedsList[position].product_token,mFeedsList[position].category?.toString())
        }


        holder?.text_id1?.text = mFeedsList[position].product_name
        holder?.special_prize_txt_id?.text = mFeedsList[position].price
